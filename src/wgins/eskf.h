...
bool ESKF<S>::Predict(const IMU& imu) {
    assert(imu.time >= current_time_);

    double dt = imu.time - current_time_;
    ...
    F.template block<3, 3>(3, 6) = -R_.matrix() * SO3::hat(imu.acce - ba_) * dt;  // v对theta
...
bool ESKF<S>::ObserveWheelSpeed(const Odom& odom) {
    assert(odom.time >= current_time_);
    // odom 修正以及雅可比
    ...
    // velocity obs
    double velo_l = options_.wheel_radius_ * odom.left_pulse / options_.circle_pulse_ * 2 * M_PI / options_.odom_span_;
    double velo_r =
        options_.wheel_radius_ * odom.right_pulse / options_.circle_pulse_ * 2 * M_PI / options_.odom_span_;
...
bool ESKF<S>::ObserveGps(const GNSS& gnss) {
    // GNSS 观测的修正
    assert(gnss.time >= current_time_);

    if (first_gnss_) {
        R_ = gnss.utm_pose_.so3();
        p_ = gnss.utm_pose_.translation();
        first_gnss_ = false;
        current_time_ = gnss.time;
        return true;
    }
