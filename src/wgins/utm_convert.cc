...
bool LatLon2UTM(const Vector2d& latlon, UTMCoordinate& utm_coor) {
    long zone = 0;
    char char_north = 0;
    long ret = Convert_Geodetic_To_UTM(latlon[0] * kDEG2RAD, latlon[1] * kDEG2RAD, &zone, &char_north, &utm_coor.xy_[0], &utm_coor.xy_[1]);
...
bool ConvertGps2UTM(GNSS& gps_msg, const Vector2d& antenna_pos, const double& antenna_angle, const Vector3d& map_origin) {
    ...
    // TWG 转到 TWB
    Sophus::SE3d TBG(Sophus::SO3d::rotZ(antenna_angle * kDEG2RAD), Vector3d(antenna_pos[0], antenna_pos[1], 0));
